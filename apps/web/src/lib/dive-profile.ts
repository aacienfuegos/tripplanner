...
export function synthesizeProfile(input: SynthesizeProfileInput): ProfileSample[] {
...
export interface DiveProfileSummary {
  depthMax: number;
  bottomTime: number;
  safetyStopMinutes: number | null;
  waterTemp: number | null;
...
export function resolveDiveProfile(dive: DiveProfileSummary, storedSamples: ProfileSample[]): ProfileSample[] {
  if (storedSamples.length > 0) return storedSamples;
  return synthesizeProfile({
    depthMax: dive.depthMax,
    bottomTimeMinutes: dive.bottomTime,
